/*
 *    class to create capacity line chart
 */

class StorageQuantityChart {
  ...
  updateVis() {
    ...
    vis.y.domain([
      d3.min(vis.dataFiltered, (c) => d3.min(c.values, (d) => d.storage)) - 5,
      d3.max(vis.dataFiltered, (c) => d3.max(c.values, (d) => d.storage)),
    ]);
    ...
    //   vis.lines = d3.select(vis.chartLocation).selectAll(".line");
    //   // console.log(Array.from(vis.linesTest));
    ...
    //           (vis.xDate = vis.mouse[0]), //x.invert removed
    ...
    //           (vis.beginning = 0),
    //             (vis.end = vis.lines[i].getTotalLength()),
    //             (vis.target = null);

    //           while (true) {
    //             vis.target = Math.floor((vis.beginning + vis.end) / 2);
    //             vis.pos = vis.lines[i].getPointAtLength(vis.target);
    //             if (
    //               (vis.target === vis.end || vis.target === vis.beginning) &&
    //               vis.pos.x !== vis.mouse[0]
    //             ) {
    //               break;
    //             }
    //             if (vis.pos.x > vis.mouse[0]) vis.end = vis.target;
    //             else if (vis.pos.x < vis.mouse[0]) vis.beginning = vis.target;
    ...
    //             .select("text")
    //             .text(
    //               vis.y.invert(vis.pos.y).toFixed(2) +
    //                 " " +
    ...
    //       //   )
    //       //   .style("display", "block")
    //       //   .style("left", vis.mouse[0] + 85 + "px")
    //       //   .style("top", vis.mouse[1] + 10 + "px")
    ...
    //       //     vis.val = d.reservoir + ":" + vis.y.invert(vis.pos.y).toFixed(2);
    ...
    //             d3.select(this)
    //               .text(vis.xDate.toLocaleDateString("pt-PT"))
    //               .attr("font-size", "10px");

    //             return (
    //               "translate(" +
    //               (vis.x(d.values[vis.idx - 1].date) + 5) +
    //               "," +
    //               HEIGHT +
    //               ")"
    //             );
    ...
    //           "," +
    //           vis.y(d.values[vis.idx - 1].storage) +
    //           ")"
    //         );
    //       });
    //   });
